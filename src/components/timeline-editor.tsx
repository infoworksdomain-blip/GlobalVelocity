...
export function TimelineEditor({ item, provenance, onClose, onDone }: { item: Item | null; provenance: Record<string, unknown>; onClose: () => void; onDone: () => void }) {
  ...
  useEffect(() => { if (!item) return; setSlides(((provenance.slides as Slide[]) ?? []).map((s) => ({ ...s }))); setBeats([...item.on_screen_text]); const m = provenance.meme as { top?: string; bottom?: string } | undefined; setMeme({ top: m?.top ?? "", bottom: m?.bottom ?? "" }); setOverlay(item.overlay_style ?? {}); }, [item, provenance]);
  if (!item) return null;
  ...
  const save = () => run(async () => {
    await api(`/content/${item.id}`, { method: "PATCH", json: { ...(isSlides ? { slides, on_screen_text: slides.map((s) => s.title) } : {}), ...(isBeats ? { on_screen_text: beats } : {}), ...(isMeme ? { meme_top: meme.top, meme_bottom: meme.bottom } : {}), ...(isBeats ? { overlay_style: Object.keys(overlay).length ? overlay : null } : {}) } });
