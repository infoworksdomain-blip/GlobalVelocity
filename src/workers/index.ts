...
async function buildRenderCtx(item: typeof contentItems.$inferSelect) {
  ...
  return { acc, profile, trend, ctx: { prepaid: (item.provenance as { studio?: string }).studio === "video", requestedSeconds: (item.provenance as { requested_seconds?: number }).requested_seconds, itemId: item.id, workspaceId: item.workspaceId, accountId: acc.id, format: item.format, profile: profile.data, screenshot, demoVideo, character: character ? { name: character.name, referenceImages: character.referenceImages.map((k) => (k.startsWith("data:") || k.startsWith("http") ? k : publicUrl(k)!)), voiceId: character.voiceId } : null, clip: clip ? { storageKey: clip.storageKey, licenceType: clip.licenceType, durationMs: clip.durationMs } : null, trend: trend?.recipe ?? null } };
}
