...
export default function Blitz() {
  ...
  const [editing, setEditing] = useState<Item | null>(null); const [scheduling, setScheduling] = useState<Item | null>(null); const [socials, setSocials] = useState<Social[]>([]); const [kept, setKept] = useState(0);
  ...
  useEffect(() => { const h = (e: KeyboardEvent) => { if (editing || scheduling || (e.target as HTMLElement)?.tagName === "INPUT" || (e.target as HTMLElement)?.tagName === "TEXTAREA") return; if (e.key === "ArrowRight" || e.key === "k") swipe("keep"); if (e.key === "ArrowLeft" || e.key === "s") swipe("skip"); if (e.key === "z" || e.key === "u") undo(); if (e.key === "e" && top) setEditing(top); if (e.key === "c" && top) setScheduling(top); }; window.addEventListener("keydown", h); return () => window.removeEventListener("keydown", h); });
  // touch swipe
