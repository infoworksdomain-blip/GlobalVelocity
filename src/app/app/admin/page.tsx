...
export default function Admin() {
  ...
  return (
    <div className="space-y-5">{wall}
      <div><h1 className="page-title">Admin panel</h1><p className="text-sm text-[var(--color-muted)] mt-1">Manage every account, key, integration and platform library.</p></div>
      <div className="flex flex-wrap gap-1 border-b border-[var(--color-hairline)]">{TABS.map(([v, label, Icon]) => <button key={v} onClick={() => setView(v)} className={`flex items-center gap-2 px-3.5 py-2.5 text-sm font-medium border-b-2 -mb-px ${view === v ? "border-brand-600 text-brand-700" : "border-transparent text-[var(--color-muted)] hover:text-[var(--color-text)]"}`}><Icon className="h-4 w-4" />{label}</button>)}</div>

      {!data ? <div className="py-20 grid place-items-center"><Spinner /></div> : data.error ? <p className="text-[var(--color-danger)]">{String(data.error)}</p> : (
        <>
          {view === "overview" && (() => { const o = data as unknown as { users: number; plans: Record<string, number>; content: { n: number; today: number }; posts: { n: number; published: number; failed: number }; credits_used: number; mode: string }; return (
            <div className="space-y-4">
              <div className="grid gap-3 grid-cols-2 md:grid-cols-4">
                {[["Users", fmtNum(o.users), "total signups"], ["Paying accounts", String(o.plans.starter + o.plans.growth + o.plans.pro), `${o.plans.free} free`], ["Content items", `${fmtNum(o.content.n)}`, `${o.content.today} today`], ["Posts published", fmtNum(o.posts.published), `${o.posts.failed} failed`]].map(([l, v, s]) => <div key={l} className="kpi"><div className="text-xs font-medium text-[var(--color-muted)]">{l}</div><div className="mt-1 text-2xl font-extrabold tracking-tight">{v}</div><div className="text-xs text-[var(--color-faint)] mt-0.5">{s}</div></div>)}
              </div>
              <div className="grid gap-4 md:grid-cols-2">
                <div className="card p-5"><div className="label">Plan distribution</div>{(["free", "starter", "growth", "pro"] as const).map((p) => { const total = Math.max(1, o.users); return <div key={p} className="mt-2"><div className="flex justify-between text-sm"><span className="capitalize">{p}</span><b>{o.plans[p]}</b></div><div className="h-1.5 rounded-full bg-[var(--color-surface-2)] mt-1"><div className="h-1.5 rounded-full bg-brand-600" style={{ width: `${(100 * o.plans[p]) / total}%` }} /></div></div>; })}</div>
          ...
          {view === "accounts" && data.accounts !== undefined && (() => { const rows = data.accounts as unknown as Record<string, never>[]; return (
            <div className="space-y-3">
              <div className="flex gap-2"><div className="relative flex-1 max-w-sm"><Search className="absolute left-3 top-3 h-4 w-4 text-[var(--color-faint)]" /><input className="input pl-9" placeholder="Search by email, name or account id" value={q} onChange={(e) => setQ(e.target.value)} onKeyDown={(e) => e.key === "Enter" && load("accounts", q)} /></div><button className="btn-secondary" onClick={() => load("accounts", q)}>Search</button><span className="ml-auto self-center text-xs text-[var(--color-muted)]">{rows.length} accounts</span></div>
              <div className="card overflow-hidden overflow-x-auto"><table className="table"><thead><tr><th>Account</th><th>Plan</th><th>Workspaces</th><th>Saves</th><th>Credits</th><th>Socials</th><th>Published</th><th>Keys</th><th>Status</th><th></th></tr></thead><tbody>
                {rows.map((a) => <tr key={String(a.accountId)}><td><button className="text-left" onClick={() => setDetail(a as unknown as Record<string, unknown>)}><div className="font-semibold text-brand-700 hover:underline">{String(a.email)}</div><div className="text-xs text-[var(--color-faint)]">{new Date(String(a.createdAt)).toLocaleDateString()}{a.affiliateRef ? ` · ref ${String(a.affiliateRef)}` : ""}</div></button></td>
                ...
                {rows.length === 0 && <tr><td colSpan={10} className="p-8 text-center text-[var(--color-muted)]">No accounts match.</td></tr>}
              </tbody></table></div>
            </div>); })()}
          ...
          {view === "keys" && data.keys !== undefined && (() => { const d = data as unknown as { keys: Record<string, never>[]; webhooks: Record<string, never>[]; socials: { platform: string; status: string; n: number }[]; rpm: Record<string, number> }; return (
            <div className="space-y-4">
              <div className="grid gap-3 grid-cols-2 md:grid-cols-4">{[["Active API keys", d.keys.filter((k) => !k.revokedAt).length], ["Revoked", d.keys.filter((k) => k.revokedAt).length], ["Webhooks", d.webhooks.length], ["Connected socials", d.socials.reduce((a, s) => a + s.n, 0)]].map(([l, v]) => <div key={String(l)} className="kpi"><div className="text-xs font-medium text-[var(--color-muted)]">{String(l)}</div><div className="mt-1 text-2xl font-extrabold">{String(v)}</div></div>)}</div>
              <div className="card overflow-hidden"><div className="px-4 py-3 text-sm font-semibold border-b border-[var(--color-hairline)]">API keys (all accounts)</div><div className="overflow-x-auto"><table className="table"><thead><tr><th>Owner</th><th>Name</th><th>Prefix</th><th>Scopes</th><th>Rate limit</th><th>Last used</th><th>Status</th><th></th></tr></thead><tbody>
                {d.keys.map((k) => <tr key={String(k.id)}><td className="font-medium">{String(k.email)}</td><td>{String(k.name ?? "—")}</td><td className="font-mono text-xs">{String(k.prefix)}…</td><td className="max-w-[220px]"><div className="flex flex-wrap gap-1">{(k.scopes as unknown as string[]).map((s) => <Badge key={s}>{s}</Badge>)}</div></td><td className="text-[var(--color-muted)]">{d.rpm[String(k.plan)]}/min</td><td className="text-[var(--color-muted)] text-xs">{k.lastUsedAt ? new Date(String(k.lastUsedAt)).toLocaleString() : "never"}</td><td>{k.revokedAt ? <Badge tone="red">revoked</Badge> : <Badge tone="green">active</Badge>}</td><td className="text-right">{!k.revokedAt && <button className="btn-ghost px-2 py-1 text-xs text-[var(--color-danger)]" onClick={() => act({ action: "revoke_key", key_id: k.id }, "Key revoked")}>Revoke</button>}</td></tr>)}
                {d.keys.length === 0 && <tr><td colSpan={8} className="p-8 text-center text-[var(--color-muted)]">No API keys issued yet.</td></tr>}</tbody></table></div></div>
              <div className="card overflow-hidden"><div className="px-4 py-3 text-sm font-semibold border-b border-[var(--color-hairline)]">Webhook endpoints</div><div className="overflow-x-auto"><table className="table"><thead><tr><th>Owner</th><th>URL</th><th>Events</th><th>Status</th><th></th></tr></thead><tbody>
                {d.webhooks.map((w) => <tr key={String(w.id)}><td className="font-medium">{String(w.email)}</td><td className="font-mono text-xs max-w-xs truncate">{String(w.url)}</td><td className="text-xs text-[var(--color-muted)]">{(w.events as unknown as string[]).join(", ")}</td><td>{w.enabled ? <Badge tone="green">enabled</Badge> : <Badge tone="slate">disabled</Badge>}</td><td className="text-right"><button className="btn-ghost px-2 py-1 text-xs" onClick={() => act({ action: "toggle_webhook", webhook_id: w.id, enabled: !w.enabled }, "Updated")}>{w.enabled ? "Disable" : "Enable"}</button></td></tr>)}
                {d.webhooks.length === 0 && <tr><td colSpan={5} className="p-8 text-center text-[var(--color-muted)]">No webhooks registered.</td></tr>}</tbody></table></div></div>
              <div className="card p-5"><div className="label">Connected social accounts by platform</div><div className="mt-2 flex flex-wrap gap-2">{d.socials.map((s) => <Badge key={s.platform + s.status} tone={s.status === "active" ? "green" : "amber"}>{s.platform}: {s.n} {s.status.replace("_", " ")}</Badge>)}{d.socials.length === 0 && <span className="text-sm text-[var(--color-muted)]">None connected.</span>}</div></div>
            </div>); })()}
          ...
          {view === "curation" && <div className="grid gap-4 md:grid-cols-2">
            ...
            <div className="card p-5 space-y-2 md:col-span-2"><h2 className="font-semibold">Bulk upload UGC clips</h2><p className="text-xs text-[var(--color-muted)]">Select up to thousands of video files at once. Uploads go direct to storage; thumbnails and durations are extracted in the background — this page stays usable while that runs.</p>
              ...
              <p className="text-xs text-[var(--color-faint)]">This metadata applies to the whole selection. Upload files with different categories/tags in separate batches.</p>
              <button className="btn-primary" disabled={!bulkFiles.length} onClick={runBulkUpload}>Upload {bulkFiles.length || ""} clips</button>
              {bulkProgress && <div className="rounded-xl bg-[var(--color-surface-2)] p-3 text-xs space-y-1">
                ...
                <div className="h-1.5 rounded-full bg-[var(--color-surface-3)]"><div className="h-1.5 rounded-full bg-brand-600" style={{ width: `${bulkProgress.total ? (100 * (bulkProgress.uploaded + bulkProgress.uploadFailed)) / bulkProgress.total : 0}%` }} /></div>
                {bulkProgress.uploadFailed > 0 && <div className="text-[var(--color-danger)]">{bulkProgress.uploadFailed} files failed to upload and were skipped (not registered)</div>}
                ...
                {bulkProgress.failed > 0 && <div className="text-[var(--color-danger)]">{bulkProgress.failed} failed to process</div>}
              </div>}
            </div>

            <div className="card p-5 space-y-2 md:col-span-2"><h2 className="font-semibold">Bulk upload images</h2><p className="text-xs text-[var(--color-muted)]">Select up to thousands of image files at once. Uploads go direct to storage; dimensions and a thumbnail are extracted in the background.</p>
              <input type="file" accept="image/*" multiple className="input" onChange={(e) => setBulkImageFiles(Array.from(e.target.files ?? []))} />
              {bulkImageFiles.length > 0 && <p className="text-xs text-[var(--color-muted)]">{bulkImageFiles.length} files selected</p>}
              <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
              ...
              {bulkImageProgress && <div className="rounded-xl bg-[var(--color-surface-2)] p-3 text-xs space-y-1">
                ...
                <div className="h-1.5 rounded-full bg-[var(--color-surface-3)]"><div className="h-1.5 rounded-full bg-brand-600" style={{ width: `${bulkImageProgress.total ? (100 * (bulkImageProgress.uploaded + bulkImageProgress.uploadFailed)) / bulkImageProgress.total : 0}%` }} /></div>
                {bulkImageProgress.uploadFailed > 0 && <div className="text-[var(--color-danger)]">{bulkImageProgress.uploadFailed} files failed to upload and were skipped (not registered)</div>}
                ...
                {bulkImageProgress.failed > 0 && <div className="text-[var(--color-danger)]">{bulkImageProgress.failed} failed to process</div>}
              </div>}
            </div>
          </div>}

          {view === "audit" && data.audit !== undefined && <div className="card overflow-hidden"><table className="table"><thead><tr><th>When</th><th>Actor</th><th>Action</th><th>Detail</th></tr></thead><tbody>{(data.audit as unknown as { id: number; action: string; actorType: string; createdAt: string; meta: unknown }[]).map((a) => <tr key={a.id}><td className="text-[var(--color-muted)] whitespace-nowrap">{new Date(a.createdAt).toLocaleString()}</td><td>{a.actorType}</td><td className="font-mono text-xs">{a.action}</td><td className="text-[var(--color-muted)] text-xs max-w-md truncate">{JSON.stringify(a.meta)}</td></tr>)}</tbody></table></div>}
        </>
      )}
      ...
      <Modal open={!!detail} onClose={() => setDetail(null)} title="Manage account" wide>{detail && (() => { const a = detail as Any as Record<string, never>; return (
        <div className="space-y-4 text-sm">
          ...
          <p className="text-xs text-[var(--color-muted)]">Stripe customer: {a.stripeCustomerId ? String(a.stripeCustomerId) : "none"} · Renews {a.renewsAt ? new Date(String(a.renewsAt)).toLocaleDateString() : "—"}</p>
        </div>); })()}</Modal>
    </div>
  );
}
