...
export default function Characters() {
  ...
  return (
    <div>{wall}
      ...
      <p className="mt-1 text-sm text-[var(--color-muted)]">All characters are synthetic. Videos are flagged as AI-generated when published.</p>
      {!rows ? <div className="py-20 grid place-items-center"><Spinner /></div> : <div className="mt-4 grid gap-3 grid-cols-2 md:grid-cols-4 lg:grid-cols-6">{rows.map((c) => <div key={c.id} className={`card overflow-hidden ${c.locked ? "opacity-70" : ""}`}>{/* eslint-disable-next-line @next/next/no-img-element */}<img src={c.referenceImages[0]} alt={`${c.name}, synthetic AI UGC character (${c.gender ?? ""} ${c.ageRange ?? ""})`} className="aspect-square w-full object-cover" /><div className="p-3"><div className="flex justify-between items-center"><b>{c.name}</b>{c.locked ? <Badge tone="amber">{c.tier}+</Badge> : c.mine ? <Badge tone="brand">mine</Badge> : null}</div><div className="text-xs text-[var(--color-muted)]">{c.gender} · {c.ageRange}</div><div className="mt-1 flex flex-wrap gap-1">{c.styleTags.slice(0, 3).map((t) => <span key={t} className="badge bg-[var(--color-surface-2)] text-[var(--color-muted)]">{t}</span>)}</div><button className="btn-primary w-full mt-3 text-xs" disabled={c.locked} onClick={() => gen(c)}>{c.locked ? "Upgrade to use" : "Generate 5 videos"}</button>{c.mine && <button className="btn-secondary w-full mt-1 text-xs" onClick={() => run(async () => { await api(`/characters/${c.id}/images`, { method: "POST", json: {} }); refresh(); api<{ characters: C[] }>("/characters").then((r) => setRows(r.characters)); }, "Consistency pack added (12 credits)")}>+ Consistency pack ({c.referenceImages.length} views)</button>}</div></div>)}</div>}
      <Modal open={open} onClose={() => setOpen(false)} title="Create an AI influencer">
        <div className="space-y-3 text-sm"><p className="text-[var(--color-muted)]">A consistent character you can reuse across videos. Generating the reference image costs 4 credits (you have {me?.usage.credits}).</p>
