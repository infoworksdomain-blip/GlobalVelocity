...
export default function Onboarding() {
  ...
  return (
    <div className="mx-auto max-w-xl py-12">{wall}
      ...
      <p className="mt-2 text-[var(--color-muted)]">Paste your website URL. We'll read your site, understand your audience and generate your first 30 posts.</p>
      {!job ? (
        ...
        <div className="card mt-8 p-6">
          ...
          {job.status === "failed" && <div className="mt-3 text-sm text-[var(--color-danger)]">{job.error ?? "Try another URL, or build the profile manually in Settings → Profile."}<button className="btn-secondary mt-3" onClick={() => setJob(null)}>Try again</button></div>}
          {job.status === "done" && <div className="mt-3 text-sm text-[var(--color-success)]">Done — opening Velocity mode…</div>}
        </div>
      )}
      <p className="mt-6 text-xs text-[var(--color-muted)]">No website yet? <a className="underline" href="/app/settings/profile">Fill the profile manually</a>.</p>
    </div>
  );
}
