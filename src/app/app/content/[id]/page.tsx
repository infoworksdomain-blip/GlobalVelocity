...
export default function ContentDetail() {
  ...
  const [item, setItem] = useState<Detail | null>(null); const [editing, setEditing] = useState(false); const [timeline, setTimeline] = useState(false); const [sched, setSched] = useState(false); const [imgEdit, setImgEdit] = useState(false); const [vidEdit, setVidEdit] = useState(false); const [socials, setSocials] = useState<Social[]>([]);
  ...
  return (
    <div className="grid gap-6 md:grid-cols-[340px_1fr]">{wall}
      <div><MediaPreview item={item} /><div className="mt-3 flex flex-wrap gap-2"><button className="btn-primary" onClick={() => setSched(true)}>Schedule</button><button className="btn-secondary" onClick={() => setEditing(true)}>Edit</button><button className="btn-secondary" onClick={() => setTimeline(true)}>Timeline</button>{item.media.image_urls.filter(Boolean).length > 0 && <button className="btn-secondary" onClick={() => setImgEdit(true)}>Edit image</button>}{item.media.video_url && <button className="btn-secondary" onClick={() => setVidEdit(true)}>Edit video</button>}<button className="btn-secondary" disabled={rerendering} onClick={rerender}>{rerendering ? "Rendering…" : "Re-render"}</button><button className="btn-secondary" onClick={() => run(async () => { const r = await api<{ url: string }>(`/content/${id}?download=1`); window.open(r.url, "_blank"); })}>Download</button><button className="btn-secondary" onClick={() => run(() => api(`/content/${id}/similar`, { method: "POST", json: { count: 5 } }), "Generating 5 similar — check Velocity mode")}>Generate similar</button><button className="btn-secondary" onClick={() => run(async () => { await api(`/content/${id}?action=duplicate`, { method: "POST" }); refresh(); }, "Duplicated")}>Duplicate</button><button className="btn-danger" onClick={() => run(async () => { await api(`/content/${id}`, { method: "DELETE" }); refresh(); router.push("/app/content"); })}>Delete</button></div></div>
      ...
      <VideoEditModal item={vidEdit ? item : null} onClose={() => setVidEdit(false)} onDone={() => { setVidEdit(false); load(); }} />
