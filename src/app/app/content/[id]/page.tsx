  ...
  return (
    <div className="grid gap-6 md:grid-cols-[340px_1fr]">{wall}
      ...
      <div className="space-y-4">
        ...
        {m && <div className="grid grid-cols-4 gap-3">{[["Views", m.views], ["Likes", m.likes], ["Comments", m.comments], ["Shares", m.shares]].map(([l, v]) => <div key={l as string} className="kpi"><div className="text-xs text-slate-500">{l}</div><div className="text-xl font-bold">{fmtNum(v as number)}</div></div>)}</div>}
        <div className="card p-4"><div className="label">Posts</div>{item.scheduled_posts.length === 0 ? <p className="text-sm text-slate-500">Not scheduled yet.</p> : <ul className="text-sm space-y-1">{item.scheduled_posts.map((p) => <li key={p.id} className="flex justify-between"><span>{PLATFORM_LABEL[p.platform]} @{p.handle} · {new Date(p.scheduledAt).toLocaleString()}</span><span className="flex gap-2 items-center"><Badge tone={statusTone(p.status)}>{p.status}</Badge>{p.permalink && <a className="underline" href={p.permalink} target="_blank">View</a>}</span></li>)}</ul>}<Link href="/app/calendar" className="text-xs underline text-slate-500 mt-2 inline-block">Open calendar</Link></div>
        <div className="card p-4"><div className="label">Versions</div>{item.versions.length === 0 ? <p className="text-sm text-slate-500">No edits yet.</p> : <ul className="text-sm space-y-1">{item.versions.map((v) => <li key={v.id} className="flex justify-between"><span>v{v.version} · {String(v.snapshot.hook ?? "")}</span><span className="text-slate-500">{new Date(v.createdAt).toLocaleString()}</span></li>)}</ul>}</div>
        <div className="card p-4"><div className="label">Provenance</div><pre className="text-xs text-slate-600 overflow-x-auto">{JSON.stringify(item.provenance, null, 2)}</pre></div>
      </div>
