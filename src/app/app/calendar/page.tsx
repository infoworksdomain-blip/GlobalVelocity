...
import { useAction, SkeletonRows, Badge, statusTone, Modal } from "@/components/ui";
    ...
    <div>{wall}
      <div className="flex flex-wrap items-center gap-2 justify-between">
        ...
        <div className="flex items-center gap-2"><button className="btn-ghost" onClick={() => setAnchor(new Date(anchor.getFullYear(), anchor.getMonth() - 1, 1))}>‹</button><span className="font-semibold w-36 text-center">{anchor.toLocaleString(undefined, { month: "long", year: "numeric" })}</span><button className="btn-ghost" onClick={() => setAnchor(new Date(anchor.getFullYear(), anchor.getMonth() + 1, 1))}>›</button>
          <div className="flex rounded-xl border border-[var(--color-hairline)] overflow-hidden text-sm">{(["month", "week", "list"] as const).map((v) => <button key={v} className={`px-3 py-1.5 ${view === v ? "bg-brand-600 text-white" : "bg-[var(--color-surface)]"}`} onClick={() => setView(v)}>{v}</button>)}</div><button className="btn-secondary" onClick={exportIcs}>Export .ics</button><button className="btn-secondary" onClick={() => run(async () => { const r = await api<{ feed_url: string }>(`/workspaces/${workspaceId}/calendar?feed=1`); await navigator.clipboard.writeText(r.feed_url); }, "Feed URL copied — subscribe in Google/Apple Calendar")}>Subscribe</button></div>
      </div>
      {!posts ? <div className="mt-4"><SkeletonRows rows={6} /></div> : view === "list" ? (
        <div className="mt-4 card overflow-hidden overflow-x-auto"><table className="w-full text-sm"><thead className="bg-[var(--color-surface-2)] text-left"><tr><th className="p-3">When</th><th className="p-3">Platform</th><th className="p-3">Content</th><th className="p-3">Status</th></tr></thead><tbody>{posts.map((p) => <tr key={p.id} className="border-t border-[var(--color-hairline)] cursor-pointer hover:bg-[var(--color-surface-2)]" onClick={() => setSel(p)}><td className="p-3">{new Date(p.scheduledAt).toLocaleString()}</td><td className="p-3">{PLATFORM_LABEL[p.platform]} @{p.handle}</td><td className="p-3">{p.hook}</td><td className="p-3"><Badge tone={statusTone(p.status)}>{p.status}</Badge></td></tr>)}{posts.length === 0 && <tr><td className="p-6 text-[var(--color-muted)]" colSpan={4}>Nothing scheduled in this range.</td></tr>}</tbody></table></div>
      ) : (
        <div className="mt-4 grid grid-cols-7 gap-px bg-[var(--color-surface-3)] rounded-2xl overflow-hidden border border-[var(--color-hairline)]">
          {["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"].map((d) => <div key={d} className="bg-[var(--color-surface-2)] p-2 text-xs font-semibold text-[var(--color-muted)]">{d}</div>)}
          {days.map((d) => { const k = dayKey(d); const inMonth = d.getMonth() === anchor.getMonth(); return (
            <div key={k} className={`bg-[var(--color-surface)] min-h-24 p-1.5 ${inMonth ? "" : "opacity-50"} ${drag ? "hover:bg-brand-50" : ""}`} onDragOver={(e) => e.preventDefault()} onDrop={() => { if (drag) { const p = posts.find((x) => x.id === drag)!; const old = new Date(p.scheduledAt); const at = new Date(d); at.setHours(old.getHours(), old.getMinutes()); reschedule(drag, at); setDrag(null); } }}>
              <div className={`text-xs ${dayKey(new Date()) === k ? "font-bold text-brand-600" : "text-[var(--color-faint)]"}`}>{d.getDate()}</div>
              <div className="space-y-1 mt-1">{(byDay[k] ?? []).map((p) => <div key={p.id} draggable={["scheduled", "pending_approval", "failed", "held"].includes(p.status)} onDragStart={() => setDrag(p.id)} onClick={() => setSel(p)} className={`flex items-center gap-1 rounded-lg px-1.5 py-1 text-[11px] cursor-pointer border ${p.status === "published" ? "bg-[rgba(61,220,151,.1)] border-[rgba(61,220,151,.3)]" : p.status === "failed" || p.status === "held" ? "bg-[rgba(251,113,133,.1)] border-[rgba(251,113,133,.3)]" : p.status === "pending_approval" ? "bg-[rgba(251,191,36,.1)] border-[rgba(251,191,36,.3)]" : "bg-[rgba(94,161,247,.1)] border-[rgba(94,161,247,.3)]"}`}>{p.thumbnail_url && /* eslint-disable-next-line @next/next/no-img-element */ <img src={p.thumbnail_url} alt={`Thumbnail for ${p.hook ?? "scheduled post"} on ${PLATFORM_LABEL[p.platform]}`} className="h-6 w-4 rounded object-cover" />}<span className="truncate">{new Date(p.scheduledAt).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })} {PLATFORM_LABEL[p.platform]?.slice(0, 2)} {p.hook}</span></div>)}</div>
            </div>); })}
        </div>
      )}
      <Modal open={!!sel} onClose={() => setSel(null)} title="Scheduled post">{sel && <div className="space-y-3 text-sm">
        ...
        {sel.lastError && <p className="text-[var(--color-danger)]">{sel.lastError}</p>}{sel.permalink && <a className="underline" href={sel.permalink} target="_blank">View post</a>}
