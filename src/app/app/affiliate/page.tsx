...
export default function Affiliate() {
  ...
  return (
    <div className="max-w-3xl space-y-4">{wall}<h1 className="text-2xl font-extrabold">Affiliate program</h1>
      {a && !a.affiliate ? <div className="card p-5"><p className="text-sm text-slate-600">Earn 30% lifetime commission on every payment from people you refer. Choose your referral code.</p><div className="mt-3 flex gap-2"><input className="input" placeholder="yourname" value={code} onChange={(e) => setCode(e.target.value)} /><input className="input" placeholder="Payout email (PayPal)" value={email} onChange={(e) => setEmail(e.target.value)} /><button className="btn-primary" disabled={busy || code.length < 3} onClick={() => run(async () => { await api("/affiliate", { method: "POST", json: { code, payout_email: email || undefined } }); load(); }, "You're in!")}>Join</button></div></div>
      : a?.affiliate && <>
        <div className="card p-5"><div className="label">Your link</div><code className="text-sm bg-slate-100 px-2 py-1 rounded">{a.affiliate.link}</code><p className="text-xs text-slate-500 mt-2">{a.affiliate.commissionPct}% lifetime · 60-day cookie · payouts monthly, $50 minimum{a.affiliate.payoutEmail ? ` to ${a.affiliate.payoutEmail}` : ""}.</p></div>
        <div className="grid grid-cols-2 md:grid-cols-5 gap-3">{[["Signups", a.stats?.signups], ["Paying", a.stats?.referred], ["Earned", usd(a.stats?.earned ?? 0)], ["Pending", usd(a.stats?.pending ?? 0)], ["Paid", usd(a.stats?.paid ?? 0)]].map(([l, v]) => <div key={l as string} className="kpi"><div className="text-xs text-slate-500">{l}</div><div className="text-xl font-bold">{v ?? 0}</div></div>)}</div>
        <div className="card overflow-hidden"><table className="w-full text-sm"><thead className="bg-slate-50 text-left"><tr><th className="p-3">Date</th><th className="p-3">Payment</th><th className="p-3">Commission</th><th className="p-3">Status</th></tr></thead><tbody>{(a.commissions ?? []).map((c) => <tr key={c.id} className="border-t border-slate-100"><td className="p-3">{new Date(c.createdAt).toLocaleDateString()}</td><td className="p-3">{usd(c.amountCents)}</td><td className="p-3">{usd(c.commissionCents)}</td><td className="p-3"><Badge tone={c.status === "paid" ? "green" : "amber"}>{c.status}</Badge></td></tr>)}{!a.commissions?.length && <tr><td className="p-6 text-slate-500" colSpan={4}>No commissions yet.</td></tr>}</tbody></table></div>
      </>}
    </div>
  );
}
