...
export default function Trends() {
  ...
  return (
    <div>{wall}
      ...
      {!me?.plan.limits.trendRemix && <p className="mt-2 text-sm text-amber-700">Recipes and remixing unlock on paid plans — you can browse hooks for free.</p>}
      {!rows ? <div className="py-20 grid place-items-center"><Spinner /></div> : <div className="mt-4 grid gap-3 md:grid-cols-3 lg:grid-cols-4">{rows.map((t) => <div key={t.id} className="card p-4 flex flex-col cursor-pointer hover:shadow-md" onClick={() => setSel(t)}><div className="flex gap-2 items-center"><Badge>{PLATFORM_LABEL[t.platform]}</Badge><Badge tone="green">🔥 {Math.round(t.velocity_score * 100)}</Badge></div><p className="mt-2 font-semibold line-clamp-3">{t.hook_text}</p><p className="mt-1 text-xs text-slate-500">{t.format_type?.replace(/_/g, " ")} · {fmtNum(t.metrics.views)} views</p><div className="mt-auto pt-3 flex flex-wrap gap-1">{t.niche_tags.slice(0, 3).map((n) => <span key={n} className="badge bg-slate-100 text-slate-600">{n}</span>)}</div></div>)}</div>}
      <Modal open={!!sel} onClose={() => setSel(null)} title="Trend"> {sel && <div className="text-sm space-y-3"><p className="text-lg font-bold">{sel.hook_text}</p><p className="text-slate-500">{PLATFORM_LABEL[sel.platform]} · {fmtNum(sel.metrics.views)} views · <a className="underline" href={sel.post_url} target="_blank" rel="noreferrer">view original</a></p>
        {sel.locked ? <p className="text-amber-700">Upgrade to see the recipe and remix this trend.</p> : sel.recipe && <><div className="label">Recipe</div><ol className="space-y-1">{sel.recipe.structure.map((s, i) => <li key={i} className="flex gap-2"><span className="w-10 text-slate-400">{s.seconds}s</span><span><b>{s.segment}</b>{s.text_slot ? ` — "${s.text_slot}"` : ""}</span></li>)}</ol><p className="text-slate-600">Style: {sel.recipe.style}. {sel.recipe.why_it_works}</p><button className="btn-primary" onClick={() => { remix(sel); setSel(null); }}>Remix for my brand</button></>}
        <p className="text-xs text-slate-400">We store trend metadata and structure only — never third-party media.</p></div>}</Modal>
    </div>
  );
}
