...
export default function SettingsLayout({ children }: { children: React.ReactNode }) {
  const p = usePathname();
  return <div><h1 className="text-2xl font-extrabold">Settings</h1><div className="mt-3 flex flex-wrap gap-1 border-b border-slate-200">{TABS.map(([k, l]) => <Link key={k} href={`/app/settings/${k}`} className={`px-3 py-2 text-sm font-medium border-b-2 ${p.includes(`/settings/${k}`) ? "border-brand-600 text-brand-700" : "border-transparent text-slate-500"}`}>{l}</Link>)}<Link href="/app/affiliate" className="px-3 py-2 text-sm font-medium text-slate-500">Affiliate</Link></div><div className="mt-6">{children}</div></div>;
}
