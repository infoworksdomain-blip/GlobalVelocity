...
export function GET() {
  ...
  const spec = {
    ...
    components: { securitySchemes: { bearer: { type: "http", scheme: "bearer" } }, schemas: {
      Generate: { type: "object", required: ["count"], properties: { count: { type: "integer", maximum: 100 }, formats: { type: "array", items: { type: "string", enum: ["ai_ugc", "human_ugc", "slideshow", "hook_demo", "meme", "remix"] } }, character_ids: { type: "array", items: { type: "string" } }, trend_ids: { type: "array", items: { type: "string" } }, language: { type: "string" } } },
