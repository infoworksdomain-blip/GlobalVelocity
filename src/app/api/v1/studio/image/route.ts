...
import { requireWorkspace, consumeCredits, refundCredits, assertCanSave, audit } from "@/lib/tenancy";
...
export const POST = route(async ({ actor, body }) => {
  ...
  if (!b.save) { await audit({ accountId: ws.accountId, workspaceId: ws.id, actorId: actor.userId, action: "studio.image_generate", meta: { credits, count: b.count, saved: false } }); return { image_keys: images, credits_used: credits }; }
  ...
  await audit({ accountId: ws.accountId, workspaceId: ws.id, actorId: actor.userId, action: "studio.image_generate", targetType: "content_item", targetId: item.id, meta: { credits, count: b.count, saved: true } });
