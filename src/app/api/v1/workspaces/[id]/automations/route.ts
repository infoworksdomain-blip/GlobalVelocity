...
export const POST = route(async ({ actor, params, body }) => {
  ...
  const b = parse(z.object({ name: z.string().min(1).max(80), mode: z.enum(["one_shot", "continuous"]).default("one_shot"), approval: z.enum(["auto", "blitz", "calendar"]).default("calendar"), config: configSchema }), body);
  const [a] = await db.insert(schema.automations).values({ workspaceId: params.id, name: b.name, mode: b.mode, approval: b.approval, config: b.config as schema.AutomationConfig, nextRunAt: b.mode === "continuous" ? new Date() : null }).returning();
  return { automation: a };
});
