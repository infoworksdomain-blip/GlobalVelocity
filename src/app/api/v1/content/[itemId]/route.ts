...
export const PATCH = route(async ({ actor, params, body }) => {
  ...
  const b = parse(z.object({ hook: z.string().max(200).optional(), script: z.string().max(5000).optional(), caption: z.string().max(3000).optional(), hashtags: z.array(z.string()).max(30).optional(), on_screen_text: z.array(z.string()).optional(), status: z.enum(["draft", "saved"]).optional(), slides: z.array(z.object({ title: z.string().max(80), body: z.string().max(200) })).max(10).optional(), meme_top: z.string().max(80).optional(), meme_bottom: z.string().max(80).optional(),
    overlay_style: z.object({ font_family: z.string().max(60).optional(), font_size_px: z.number().int().min(24).max(160).optional(), bold: z.boolean().optional(), color: z.string().regex(/^#[0-9a-fA-F]{6}$/).optional(), position: z.enum(["top", "center", "bottom"]).optional() }).nullable().optional(),
    beat_durations: z.array(z.number().positive()).max(20).optional() }), body);
  ...
  const [updated] = await db.update(schema.contentItems).set({ hook: b.hook ?? item.hook, script: b.script ?? item.script, caption: b.caption ?? item.caption, hashtags: b.hashtags ?? item.hashtags, onScreenText: b.on_screen_text ?? item.onScreenText, status: b.status ?? item.status, overlayStyle: b.overlay_style !== undefined ? b.overlay_style : item.overlayStyle, provenance: { ...item.provenance, ...(b.slides ? { slides: b.slides } : {}), ...(b.meme_top || b.meme_bottom ? { meme: { top: b.meme_top ?? (item.provenance as { meme?: { top: string } }).meme?.top, bottom: b.meme_bottom ?? (item.provenance as { meme?: { bottom: string } }).meme?.bottom } } : {}), ...(b.beat_durations ? { beat_durations: b.beat_durations } : {}) }, updatedAt: new Date() }).where(eq(schema.contentItems.id, item.id)).returning();
