 ...
 *
 * HeyGen plans cap the number of photo avatars an account can hold (as low as 3), so the talking photo
 * is created once per character and cached on characters.heygenTalkingPhotoId -- every subsequent video
 * for that character reuses it instead of burning another avatar slot.
 *
...
export async function POST(req: Request) {
  ...
  const { character_image_url, character_id, audio_base64, aspect } = (await req.json()) as {
  ...
  try {
    // 1. Reuse the character's cached talking photo if we've already created one; otherwise upload
    // the reference photo (data: URL or http(s) URL) to HeyGen and persist the id for next time.
    const existing = character_id ? await db.query.characters.findFirst({ where: eq(schema.characters.id, character_id), columns: { heygenTalkingPhotoId: true } }) : null;
    let talkingPhotoId = existing?.heygenTalkingPhotoId ?? undefined;
    if (!talkingPhotoId) {
      ...
      talkingPhotoId = tpJson.data?.talking_photo_id;
      if (!talkingPhotoId) throw new Error(`heygen talking_photo upload returned no id: ${JSON.stringify(tpJson)}`);
      if (character_id) await db.update(schema.characters).set({ heygenTalkingPhotoId: talkingPhotoId }).where(eq(schema.characters.id, character_id));
    }

    // 2. Upload our TTS audio to R2 so HeyGen can fetch it by URL
